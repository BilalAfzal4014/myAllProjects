        ...
        if (isset($variants_tokens) && sizeof($variants_tokens) > 0) {
                                            ...
                                            if ($campaign_type == Campaign::CAMPAIGN_INAPP_CODE || $campaign_type == Campaign::CAMPAIGN_PUSH_CODE) {
                                                ...
                                                if(strpos($template_content, '</body>')){
                                                    $template_content = str_replace( '</body>',
                                                        '<div style="font-size:14px;color:#a79d9d;padding:10px;margin:10px;text-align: center;">Don\'t want to receive further emails? 
                                                                    <a href="'. route('unsubscribe-email') .'?enc='. base64_encode($tracking_key) .'" target="_blank" >Unsubscribe here</a> 
                                                                <br>
                                                                OR 
                                                                Paste the below URL into your browser window.
                                                                <br>
                                                                '. $unsubscribe_url .
                                                ...
                                                $_payload_params = ['campaign_code' => $campaign_code,
                                                    'campaign_id' => $campaign_id,
            ...
            if ($campaign_delivery_type == Campaign::DELIVERY_TYPE_SCHEDULE) {
                ...
                //if (isset($campaign_rate_limit)) {
                    ...
                    $_payload = RateLimitingComponents::rateLimitingRules($campaign_id, $payload_params, $rate_limit, $duration_value, $duration_unit);
                    for ($val = 0; $val < count($_payload); $val++) {
                        $payload_interval = (isset($_payload[$val]['payload']['data']['interval'])) ? $_payload[$val]['payload']['data']['interval'] : $_payload[$val]['payload']['data']['campaign_start_date'];
