...
class NewsFeedResource implements ResourcesContract, ProcessResourceDataContract
{
    ...
    public function actionList(\Illuminate\Http\Request $request)
    {
        try {
            ...
            if ($appGroupId) {
                ...
                switch ($dataType) {
                    case 'all':
                        $response = $this->getActions('all', $appGroupId, $companyid);
                        break;
                    case 'action':
                        $response = $this->getActions('action', $appGroupId, $companyid);
                        break;
                    case 'conversion':
                        $response = $this->getActions('conversion', $appGroupId, $companyid);
                        break;
    ...
    public function getActions($dataType, $appGroupId, $companyid)
    {
        $action = array();
        $conversion = array();
        $user = array();
