...
class StatsSummary
{
    ...
    public function getCampaignStatsCount($request)
    {
        ...
        $queryChaining = DB::table('campaign')
            ...
            ->where("campaign_tracking.sent_at", ">=", $data["startDate"])
            ->where("campaign_tracking.sent_at", "<=", $data["endDate"]);
    ...
    public function getConversionCount($request)
    {
        ...
        $queryChaining = DB::table('campaign')
            ...
            ->where('campaign.app_group_id', '=', \Request::user()->currentAppGroup()->id)
            ->where("app_user_activity.created_at", ">=", $data["startDate"])
            ->where("app_user_activity.created_at", "<=", $data["endDate"]);
    ...
    public function getNewsFeedCount($request)
    {
        ...
        $queryChaining1 = DB::table('news_feed')
            ...
            ->where("link_tracking.created_at", ">=", $data["startDate"])
            ->where("link_tracking.created_at", "<=", $data["endDate"]);
    ...
    public function getUserStatsCount($request)
    {
        ...
        if ($data["startDate"] != null && $data["startDate"] != "") {
            $queryChain->where("created_at", ">=", $data["startDate"]);
        }

        if ($data["endDate"] != null && $data["endDate"] != "") {
            $queryChain->where("created_at", "<=", $data["endDate"]);
        }
