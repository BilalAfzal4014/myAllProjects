                        ...
                        if (isset($app_users) && count($app_users) > 0) {
                            ...
                            if ($data['type'] == Campaign::CAMPAIGN_INAPP_CODE || $data['type'] == Campaign::CAMPAIGN_PUSH_CODE) {
                                ...
                                if(isset($tokens_data) && count($tokens_data) > 0){
                                    ...
                                    /*
                                     * $user_device_type = (isset($_app_users[0]['device_type'])) ? $_app_users[0]['device_type'] : "";
                                     * if($data['type'] == Campaign::CAMPAIGN_INAPP_CODE &&
                                        (
                                            $data['device_type'] == strtolower(AppPlatforms::PLATFORM_ANDROID)
                                            ||  (
                                                    $data['device_type'] == strtolower( AppPlatforms::PLATFORM_UNIVERSAL )
                                                    && $user_device_type == strtolower(AppPlatforms::PLATFORM_ANDROID)
                                            )
                                        )
                                    ){
                                        $notification = null;
