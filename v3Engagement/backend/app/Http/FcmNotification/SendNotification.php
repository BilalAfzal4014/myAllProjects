...
namespace App\Http\FcmNotification;
...
use App\Http\FcmNotification\NotificationRequest;
use LaravelFCM\Sender\HTTPSender;


class SendNotification extends HTTPSender
{
     ...
     */
    public function sendTo($to, Options $options = null, PayloadNotificationDataBinding $notification = null, PayloadData $data = null)
    {
        $response = null;
        if (is_array($to) && !empty($to)) {
            $partialTokens = array_chunk($to, self::MAX_TOKEN_PER_REQUEST, false);
            foreach ($partialTokens as $tokens) {
                $request = new NotificationRequest($tokens, $options, $notification, $data);
            ...
            $request = new NotificationRequest($to, $options, $notification, $data);
            $responseGuzzle = $this->post($request);
     ...
     */
    public function sendToGroup($notificationKey, Options $options = null, PayloadNotificationDataBinding $notification = null, PayloadData $data = null)
    {
        $request = new NotificationRequest($notificationKey, $options, $notification, $data);
     ...
     */
    public function sendToTopic(Topics $topics, Options $options = null, PayloadNotificationDataBinding $notification = null, PayloadData $data = null)
    {
        $request = new NotificationRequest(null, $options, $notification, $data, $topics);
