...
class CampaignValidation
{
    ...
    {
        $campaigCheck = Campaign::find($campaignId);
        if (!$campaigCheck) {
            $error = "Campaign Not Valid.";
            Log::error($error);
            throw new \Exception($error, 403);
        }
        $companyCheck = AppGroup::find($campaigCheck->app_group_id)->company;
        if (!$companyCheck) {
            $error = "Company id is not valid.";
            Log::error($error);
            throw new \Exception($error, 403);
        }
        if ((bool)$companyCheck->is_active === false) {
            $error = "Company has been deactivated";
            Log::error($error);
            throw new \Exception($error, 403);
        }

        if ((bool)$campaigCheck->is_deleted === true) {
