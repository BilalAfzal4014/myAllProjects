<template>
  <swiper v-if="sliderData.length > 0" :options="galleryOption" class="swiper">
    <swiper-slide v-for="(gallery,index) in sliderData" :key="index">
      <GalleryCard :galleryImage="gallery.image_name ?? ''" />
    </swiper-slide>
    <swiper-slide>
      <div class="btn-package-detail-wrap">
        <button class="btn-browse-more" @click="$emit('navigateToTab', 'gallery')">
