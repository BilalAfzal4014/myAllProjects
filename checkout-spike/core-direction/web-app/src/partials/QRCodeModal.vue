<template>
  <div id="qr_code_modal" class="wrapper">
    <!-- inspire-your-network-modal -->
    <div id="inspire-your-network-modal" class="custom-modal m-auto">
      <div class="modal-center">
        <div class="modal-outer-box">
          <div class="modal-inner-box">
            <div class="modal-header">
              <div class="btn-modal-close ml-auto" @click="closeModal">
                <svg fill="none" height="36" viewBox="0 0 36 36" width="36" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M30.7336 5.26636C23.7103 -1.75545 12.2885 -1.75545 5.2652 5.26636C-1.75507 12.2882 -1.75507 23.713 5.2652 30.7348C8.77685 34.245 13.3885 35.9992 18.0002 35.9992C22.6119 35.9992 27.222 34.2449 30.7336 30.7348C37.7555 23.7131 37.7555 12.2882 30.7336 5.26636ZM25.4271 23.3063C26.0139 23.8931 26.0139 24.8415 25.4271 25.4283C25.1345 25.7209 24.7503 25.868 24.3661 25.868C23.982 25.868 23.5977 25.7209 23.3051 25.4283L18.0001 20.1218L12.6966 25.4267C12.4025 25.7194 12.0183 25.8665 11.6356 25.8665C11.2515 25.8665 10.8672 25.7194 10.5746 25.4267C9.98785 24.84 9.98785 23.89 10.5746 23.3048L15.8781 17.9998L10.5731 12.6948C9.98637 12.1081 9.98637 11.1581 10.5731 10.5729C11.1584 9.98612 12.1083 9.98612 12.6951 10.5729L18.0001 15.8778L23.305 10.5729C23.8918 9.98612 24.8402 9.98612 25.427 10.5729C26.0137 11.1581 26.0137 12.1081 25.427 12.6948L20.122 17.9998L25.4271 23.3063Z"
                    fill="#690fad"
                  />
                </svg>
              </div>
            </div>
            <div class="modal-body px-5">
              <p class="modal-title text-center mt-5 username font-monsterrat">
                {{ $data.username }}
              </p>
              <div class="form-container mx-auto">
                <p class="modal-desc">
                  This is a unique QR code against your user ID
                </p>
                <div class="block">
                  <img class="float-left" src="/assets/images/left-corner-union.png">
                  <img class="float-right	" src="/assets/images/right-corner-union.png">
                </div>
                <div class="clear-both" />
                <div>
                  <img :src="$data.qrCode" class="qr_code_image">
                </div>
                <div class="clear-both" />
                <div class="block">
                  <img class="float-left" src="/assets/images/bottom-left-corner-union.png">
                  <img class="float-right	" src="/assets/images/bottom-right-corner-union.png">
                </div>
                <div class="clear-both" />
                <p class="text-center clear-both my-5">
                  Please present this QR code for activity <br>
                  and event booking access.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import qrCodeMixin from "@/mixin/qrCodeMixin";

export default {
    name: "QRCodeModal",
    mixins: [qrCodeMixin],
    methods: {
        closeModal() {
            const body = document.querySelector("body");
            body.classList.remove("overflow-y-hidden");
            this.$emit("hide", "hide QR code");
        }
    }
};
</script>

<style scoped>
.username {
  font-style: normal;
  font-weight: bold;
  font-size: 18px;
  line-height: 21px;
}

#inspire-your-network-modal .modal-title {
  font-size: 18px !important;
  font-weight: bold !important;
  margin-bottom: 0px;

}

img.qr_code_image {
  height: 200px;
  max-width: 250px;
  margin: 0 auto;
}
#qr_code_modal .modal-body{
  padding-bottom: 45px;
}
</style>