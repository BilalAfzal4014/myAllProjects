...
module.exports = class UpdateProfileImageUseCase extends BaseUseCase {
    constructor(payLoad) {
        super();
    ...
    fetchPreviousImageFromDB() {
        return FosUserUserRepo.findById(this.payLoad.user_id)
            .then((user) => {
                return this.payLoad.previousFileName = user.company_logo;
            });
    }
    ...
    prepareData() {
        return {company_logo: this.payLoad.fileName};
    }
}