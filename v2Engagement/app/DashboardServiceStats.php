...
trait DashboardServiceStats
{
    ...
    {
        if ($this->isCompany === true) {
            return LinkTracking::where('rec_type', 'newsfeed')
    ...
    {
        if ($this->isCompany === true) {
            return NewsFeedImpression::whereIn('news_feed_id', $this->newsfeedIds)
                ->whereBetween($column, $range)->get();
        }

        return NewsFeedImpression::whereBetween($column, $range)->get();
    }
