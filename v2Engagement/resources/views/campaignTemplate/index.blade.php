...
@section('searchBar')
    <div class="tp_BreadCrumb_list_sec clearfix">
        <label class="sec_tp_title"> Campaign Template </label>
            ...
            <div class="uder_deta_dropdown">
                ...
                <div class=" inp_select">
                    <select id="dashboard_quick_action">
                        ...
                        <option value="{{ route('campaignTemplates.create') }}">Add New User</option>
                    </select>
                </div>
            </div>
        </div>
...
@section('content')
    @include('partials.left-scroll-bar-campaignTemplate')
    <style>
        /* datatable styling */
            ...
            <div class="list_table_body scrollbar_content mCustomScrollbar _mCS_1">
                ...
                <table cellspacing="0" cellpadding="0" padding-right:10px; id="userTable">
                    ...
                    @foreach ($CampaignTemplate as $CampaignTemplates)
                        <tr>
                            <td>{{ $CampaignTemplates->id }}</td>
                            <td>{{ $CampaignTemplates->title }}</td>
                            <td>{{ $CampaignTemplates->type }}</td>
                            <td>{{ $CampaignTemplates->created_at->format('F d, Y h:ia') }}</td>
                            <td style="width:6%;">
    ...
    <script>
        $(document).ready(function () {
            ...
            function getUserListing(filter, filterType) {
                console.log('filter');
                var url = baseUrl + '/campaignTemplates/listing/' + filter;
                console.log('url', url);
